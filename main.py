...
class MatchViewer(QDialog):
    ...
    def search_matches(self):
        ...
        def match_condition(team, name, match):
            ...
            if team == "team_a":
                return name in team_a
            elif team == "team_b":
                return name in team_b
            elif team == "any":
...
class CharacterManager(QWidget):
    ...
    def update_team_stats(self):
        ...
        def format_stat(value, stat_name):
            formatted_value = f"{value:.1f}"
            if value >= 100:
                return f"<span style='color: red; font-weight: bold;'>{stat_name}: {formatted_value}</span>"
            return f"{stat_name}: {formatted_value}"
        ...
        team_a_text = "进攻方: " + " | ".join(
            format_stat(team_a_stats[attr], attr) for attr in ["2RL", "2.5RL", "3RL", "3.5RL", "4RL"]
        )
        team_b_text = "防守方: " + " | ".join(
            format_stat(team_b_stats[attr], attr) for attr in ["2RL", "2.5RL", "3RL", "3.5RL", "4RL"]
        )
